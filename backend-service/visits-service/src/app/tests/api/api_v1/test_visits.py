...
from ...utils.utils import random_integer, random_datetime, random_lower_string
...
def test_read_visit(
    ...
    response = client.get(
        f"{settings.API_V1_STR}/visits/{visit.id}", headers=""
    )
...
def test_create_visit(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/visits/", headers="", json=data
    )
